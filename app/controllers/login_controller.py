...
class LoginController(object):
   ...
   def validate_credentials_login(self, credentials: LoginCredentials) -> str | bool | dict:
      try:
         with Session(engine) as session:
            statement = select(Users.password).where(Users.email == credentials.user_name)
            result = session.exec(statement).first()
            if result == create_sha256_hash(credentials.password):
               payload = PayloadCredentials(user_name=credentials.user_name, role="user")
